...
import numpy as np
import os
from collections import defaultdict # Used to easily append to lists within a dictionary
...
all_inter_provider_stats = [] # Renamed for clarity, as it will now store medians
...
for stat in stats_list:
    ...
    # Initialize a defaultdict to store lists of Jaccard scores for each provider
    # This is crucial for calculating the median later
    provider_jaccard_scores_list_vs_others = defaultdict(list)
        ...
        for j in range(i + 1, len(all_samples)):
            ...
            if authority1 is not None and authority2 is not None and authority1 != authority2:
                ...
                if not pd.isna(score):
                    ...
                    # Append the score to the list for both authorities involved in the comparison
                    provider_jaccard_scores_list_vs_others[authority1].append(score)
                    provider_jaccard_scores_list_vs_others[authority2].append(score)

    print("\nProvider Jaccard Scores List (vs. Others, sample):", {k: v[:5] for k,v in provider_jaccard_scores_list_vs_others.items()}) # Print first 5 scores
    print("Found any inter-group matching pairs:", found_any_inter_matching_pairs)

    # --- Calculate Medians for Inter-Provider Scores ---
    provider_jaccard_medians_vs_others = {}
    for provider, scores in provider_jaccard_scores_list_vs_others.items():
        if scores: # Ensure there are scores to calculate a median
            provider_jaccard_medians_vs_others[provider] = np.median(scores)
    print("Provider Jaccard Medians (vs. Others):", provider_jaccard_medians_vs_others)
    # --- End Calculate Medians ---

    # Convert the results to a DataFrame for plotting INTER-PROVIDER MEDIANS
    # Changed column name to 'Median Jaccard Score'
    inter_provider_median_df = pd.DataFrame(list(provider_jaccard_medians_vs_others.items()), columns=['Provider', 'Median Jaccard Score'])
    inter_provider_median_df['Statistic'] = stat # Add a column to identify the statistic

    # Append to the list for combined plotting
    if not inter_provider_median_df.empty:
        all_inter_provider_stats.append(inter_provider_median_df)
    else:
        print(f"\nNo inter-provider Jaccard medians found for {stat}.")


# --- Combined Plotting of All Inter-Provider Medians as a Dot Plot ---
if all_inter_provider_stats:
    # Concatenate all DataFrames into one for plotting
    combined_df = pd.concat(all_inter_provider_stats)
    ...
    # Determine dynamic x-axis upper limit based on median scores
    max_jaccard_score = combined_df['Median Jaccard Score'].max()
    ...
    sns.scatterplot(
        x='Median Jaccard Score', # Changed to Median Jaccard Score
        y='Statistic',
        hue='Provider',
        style='Provider',
        data=combined_df,
    ...
    plt.title('Median Jaccard Scores: Each Provider vs. All Other Providers (Dot Plot)') # Updated title
    plt.xlabel('Median Jaccard Score') # Updated x-axis label
    ...
    save_path_combined_median_dot_plot = os.path.join(results_dir,'dotplot_ALL_STATS_COMBINED_MEDIANS_VS_OTHERS.svg') # New filename
    plt.savefig(save_path_combined_median_dot_plot)
    plt.close()
    print(f"\nSaved combined median dot plot: {save_path_combined_median_dot_plot}")
else:
    print("\nNo data to generate a combined median dot plot.")
