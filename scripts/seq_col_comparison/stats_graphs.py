...
pep_df['jaccard_names'] = pd.to_numeric(pep_df['jaccard_names'], errors='coerce')
    ...
    for sample2 in all_samples:
        ...
        if sample1 == sample2:
            ...
            if not comparison.empty:
                similarity_score = comparison['jaccard_names'].iloc[0]
...
sns.heatmap(heatmap_data, annot=True, cmap='viridis', fmt=".2f", linewidths=.5, cbar_kws={'label': 'Jaccard Similarity'})
plt.title('Jaccard Similarity Heatmap (O(n^2) Construction)')
